...
type GfToken struct {
	...
	ServerName 		 string   // GoFrame 服务名称

	CacheMode 		 int8 	  // 缓存模式: 1 gcache ，2 gredis 默认1

	CacheKey 		 string   // 缓存key

	Timeout 		 int	  // 超时时间 默认10天（毫秒）

	MaxRefresh 		 int	  // 缓存刷新时间 默认为超时时间的一半（毫秒）

	TokenDelimiter   string	  // Token分隔符

	EncryptKey 		 []byte	  // Token加密key

	AuthFailMsg 	 string   // 认证失败中文提示

	MultiLogin		 bool	  // 是否支持多端登录，默认false

	GlobalMiddleware bool	  // 是否是全局认证，兼容历史版本。这个字段的功能已经失效了。

	MiddlewareType   uint	  // 中间件类型 1 GroupMiddleware 2 BindMiddleware  3 GlobalMiddleware

	LoginPath 		 string		// 登录路径

	// 登录验证方法 return userKey 用户标识 如果userKey为空，结束执行
	LoginValidate func(r *ghttp.Request) (string, interface{})

	// 登录返回方法
	LoginResponse func(r *ghttp.Request, respData Resp)

	// 登出地址
	LogoutPath string

	// 登出验证方法 return true 继续执行，否则结束执行
	LogoutValidate func(r *ghttp.Request) bool

	// 登出返回方法
	LogoutResponse func(r *ghttp.Request, respData Resp)
	...
	Authenticator func(r *ghttp.Request) bool

	// 认证返回方法
	AuthResponse func(r *ghttp.Request, respData Resp)
}
...
func (m *GfToken) InitConfig() bool {
	...
	if m.LoginResponse == nil {
		m.LoginResponse = func(r *ghttp.Request, respData Resp) {
			if !respData.Success() {
				r.Response.WriteJson(respData)
			} else {
				r.Response.WriteJson(Ok(g.Map{
	...
	if m.LogoutValidate == nil {
		m.LogoutValidate = func(r *ghttp.Request) bool {
			return true
		}
	}

	if m.LogoutResponse == nil {
		m.LogoutResponse = func(r *ghttp.Request, respData Resp) {
			if respData.Success() {
				r.Response.WriteJson(Ok("登出成功"))
			} else {
				r.Response.WriteJson(respData)
			}
		}
	...
	if m.Authenticator == nil {
		m.Authenticator = func(r *ghttp.Request) bool {
			// 静态页面不拦截
			if r.IsFileRequest() {
				return false
			}
	...
	if m.AuthResponse == nil {
		m.AuthResponse = func(r *ghttp.Request, respData Resp) {
			if respData.Success() {
				r.Middleware.Next()
			} else {
...
// GetTokenData 通过token获取登录信息，例如: 获取用户名
...
func (m *GfToken) Login(r *ghttp.Request) {
	userKey, data := m.LoginValidate(r)
	if userKey == "" {
